...
# Локаль для русских дат (если доступна в системе)
LC_TIME_RU="ru_RU.UTF-8"

# Метки времени: сейчас и окончание
NOW="$(LC_TIME=${LC_TIME_RU} date '+%A, %d %B %Y г. %H:%M:%S (%Z)')"
END="$(LC_TIME=${LC_TIME_RU} date -v+"${minutes}"M '+%A, %d %B %Y г. %H:%M:%S (%Z)')"

# Запускаем caffeinate: предотвращаем простой и сон дисплея на заданный таймаут
# Флаги: -i (idle), -d (display), -t <секунды>
...
# Дожидаемся завершения caffeinate на всякий случай
